...
import { logger } from '../utils/logger.js'; // Import formatted logger

export const getTodos = async (req, res) => {
    try {
        const todos = await Todo.find({ userId: req.session.userId });
        // logger.info('Fetched todos', { userId: req.session.userId, count: todos.length });
        res.json(todos);
    } catch (err) {
        logger.error('Failed to fetch todos', { userId: req.session.userId, error: err.message });
        res.status(500).render('404'); // Routing error to 404 page
    }
};
